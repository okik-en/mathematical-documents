...
# Clean old index directory output so docs/index.html is authoritative
$legacyIndexDir = Join-Path $docs 'index'
...
Get-ChildItem -Path (Join-Path $root 'src') -Recurse -Filter *.typ | ForEach-Object {
  ...
  if ($relative -eq 'index.typ') {
    $outDir = $docs
    $outPath = Join-Path $docs 'index.html'
  } else {
    $outDir = Join-Path $docs $outRelative
    $outPath = Join-Path $outDir 'index.html'
  }
  if (-not (Test-Path $outDir)) { New-Item -ItemType Directory -Path $outDir | Out-Null }

  if ($relative -eq 'index.typ') {
    Write-Host "Compiling: $relative -> docs/index.html"
    $cssPath = 'style.css'
  } else {
    Write-Host "Compiling: $relative -> docs/$outRelative/index.html"
    $cssPath = '../style.css'
  }
